"""
Sarah AI Processing Engine with Pydantic and LangGraph for Agentic Tasks

This module implements an agentic document processing system for Sarah AI
using Pydantic for data validation and LangGraph for workflow management.
"""
...
class SarahAIProcessor:
    """
    Agentic AI Processing Engine for Sarah AI
    Uses Pydantic for data validation and LangGraph for workflow management
    """
    ...
    async def _call_ollama(self, session, model: str, prompt: str, image_path: str = None) -> str:
        ...
        payload = {
            ...
            "options": {
                ...
                "stop": ["\n\n", "If you cannot find the value", "respond with", "If you are unable to extract"]  # Stop tokens
            }
        }
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                if response.status == 200:
                    ...
                    if "If you are unable to extract" in response_text:
                        # Take only the first part before the repetitive text
                        response_text = response_text.split("If you are unable to extract")[0].strip()
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        workflow.add_node("extract_text_with_docling", self._node_extract_text_with_docling)
        workflow.add_node("extract_with_deepseek_ocr", self._node_extract_with_deepseek_ocr)
        workflow.add_node("extract_according_to_schema", self._node_extract_according_to_schema)
        workflow.add_node("calculate_confidence", self._node_calculate_confidence)
        
        # Define the flow
        workflow.set_entry_point("extract_text_with_docling")
        workflow.add_edge("extract_text_with_docling", "extract_with_deepseek_ocr")
        workflow.add_edge("extract_with_deepseek_ocr", "extract_according_to_schema")
        workflow.add_edge("extract_according_to_schema", "calculate_confidence")
        workflow.add_edge("calculate_confidence", END)
        
        return workflow.compile()
    
    async def _node_extract_text_with_docling(self, state: DocumentState) -> Dict[str, Any]:
        """Node to extract text using Docling"""
        logger.info("Node: Extracting text with Granite Docling...")
        
        async with aiohttp.ClientSession() as session:
            text_content = await self._extract_text_with_docling(session, state["document_path"])
        
        return {"raw_text": text_content}
    
    async def _node_extract_with_deepseek_ocr(self, state: DocumentState) -> Dict[str, Any]:
        """Node to extract text using DeepSeek OCR"""
        logger.info("Node: Extracting with DeepSeek OCR...")
        
        async with aiohttp.ClientSession() as session:
            ocr_content = await self._extract_with_deepseek_ocr(session, state["document_path"])
        
        return {"raw_ocr": ocr_content}
    
    async def _node_extract_according_to_schema(self, state: DocumentState) -> Dict[str, Any]:
        """Node to extract data according to schema"""
        logger.info("Node: Extracting data according to schema...")
    ...
    def _node_calculate_confidence(self, state: DocumentState) -> Dict[str, Any]:
        """Node to calculate confidence score"""
        logger.info("Node: Calculating confidence...")
        
        confidence = self._calculate_confidence(state["extracted_data"], state["schema"])
        ...
        processing_result = ProcessingResult(
            ...
            raw_ocr=state["raw_ocr"],
            processing_timestamp=datetime.utcnow().isoformat(),
            schema_used=state["schema"]
        )
    ...
    async def process_document_with_schema(self, document_path: str, schema: List[Dict[str, str]]) -> ProcessingResult:
        """
        Process a document according to a user-defined schema using the agentic workflow
        """
...
async def test_with_real_models():
    """
    Test the agentic processing engine with actual Ollama models
    """
    logger.info("=== Sarah AI Agentic Processing Engine with Real Models ===\n")
